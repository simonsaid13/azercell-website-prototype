(function () {
  ...
  function tariffComparisonHref() {
    var path = '/compare/';
    var resolved = (window.SiteRegistry && window.SiteRegistry.href)
      ? window.SiteRegistry.href(path)
      : path;
    if (resolved.indexOf('/planned/') === 0) return resolved;
    return path + '?billing=prepaid&lang=' + selectedLanguage.toLowerCase() + '&variant=' + headerVariant;
  }
