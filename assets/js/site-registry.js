...
(function (global) {
  ...
  var PAGE_REGISTRY = [
    ...
    { path: '/nav-lab/', title: 'Navigation review probe', parent: null, branch: 'internal', status: 'built',
      links: ['/nav-lab/transfer-number/', '/compare/'],
      note: 'Hidden route. Isolated header/menu review; not linked from public navigation. Does not replace the live site header.' },
    { path: '/nav-lab/transfer-number/', title: 'Transfer your number probe', parent: '/nav-lab/', branch: 'internal', status: 'built',
      note: 'Hidden route. Isolated product-page scenario using the review header; not the live Transfer your number page.' },
    { path: '/compare/', title: 'Tariff comparison probe', parent: null, branch: 'internal', status: 'built',
      links: ['/nav-lab/'],
      note: 'Review compare at /compare/. Linked from nav-lab and from personal prepaid/postpaid Compare buttons. Does not replace the live /tariffs/compare/ page.' },
    ...
    { path: '/', title: 'B2C Homepage', parent: null, branch: 'b2c', status: 'built',
      links: ['/tariffs/', '/tariffs/internet/', '/tariffs/roaming/', '/devices/', '/campaigns/',
              '/apps/', '/support/', '/about/', '/business/', '/compare/', 'https://kabinetim.azercell.com/my/login'] },
    ...
    { path: '/tariffs/mobile/', title: 'Tariffs', parent: '/tariffs/', branch: 'b2c', status: 'built',
      links: MOBILE_TARIFF_DETAIL_PATHS.concat(['/tariffs/mobile/prepaid/archive/', '/tariffs/internet/', '/compare/']) },
    ...
    { path: '/business/', title: 'Business homepage', parent: null, branch: 'b2b', status: 'built',
      links: ['/', '/about/', '/business/mobile/', '/business/connectivity/', '/business/digital-solutions/',
              '/business/iot/', '/business/fleet-field-operations/', '/business/automation-management/',
              '/business/customer-engagement/', '/business/campaigns/', '/business/support/',
              '/business/mobile/tariffs/', '/business/mobile/internet/', '/business/mobile/wifi/',
              '/business/mobile/roaming/', '/business/mobile/campaigns/', '/business/fixed/leased-line/',
              '/business/cloud/', '/about/contact/', 'https://biznes.azercell.com'] },
  ...
  function href(path) {
    ...
    var q = path.indexOf('?');
    var pathname = q >= 0 ? path.slice(0, q) : path;
    var query = q >= 0 ? path.slice(q) : '';
    var page = byPath[pathname];
    if (page && page.status === 'built') return pathname + query;
  ...
  /** Personal compare lab at /compare/. Prepaid vs postpaid is the shareable tab. */
  function compareLabHref(billing) {
    var base = href('/compare/');
    if (base.indexOf('/planned/') === 0) return base;
    if (!billing || billing === 'all') return base;
    var type = billing === 'postpaid' ? 'postpaid' : 'prepaid';
    var join = base.indexOf('?') >= 0 ? '&' : '?';
    return base + join + 'billing=' + encodeURIComponent(type);
  }

  /** Compare button on personal prepaid/postpaid cards and detail pages. */
  function tariffCompareHref(id) {
    return compareLabHref(id === 'alfa' ? 'postpaid' : 'prepaid');
  }
  ...
  var SITE_CHROME = {
    ...
    /* B2B uses the personal footer layout with business groups and no Apps column. */
    businessFooter: {
      layout: 'personal',
      brand: { title: 'Azercell', tagline: 'Every connection opens a possibility' },
      subscribe: {
  ...
  var COMPONENT_REGISTRY = [
    ...
    },
    {
      ...
      group: 'Global chrome',
      usedOn: PERSONAL_CHROME_PATHS.concat(['/business/']),
      description: 'Personal: utility bar plus mega menus (Mobile, TV, Apps, Devices, Campaigns, Support, Company). Business uses the same header with Mobile, Connectivity, Digital solutions, Campaigns, Support, Company.',
      props: {
        layout: 'personal',
        branch: 'personal',
    ...
    },
    {
      ...
      group: 'Global chrome',
      usedOn: PERSONAL_CHROME_PATHS.concat(['/business/']),
      description: 'Shortcut bar. Personal: Internet, Tariffs, Roaming, Kinon. Business: Internet, Tariffs, Roaming, Azercell Biznes. On the transfer page it changes to a Start transfer action after the hero button scrolls away.',
    ...
    },
    {
      ...
      usedOn: ['/', '/business/'],
      description: 'Personal: five cards for getting a number, transferring, choosing a tariff, e-SIM and internet. Business: four solution cards (IoT, fleet, automation, customer engagement).',
    ...
    },
    {
      ...
      group: 'Global chrome',
      usedOn: PERSONAL_CHROME_PATHS.concat(['/business/']),
      description: 'Personal: brand, subscribe, sitemap-style columns, app download card, legal and social. Business uses the same layout with business columns and two feature cards, no Apps column.',
        ...
        slides: [
          {
            ...
            actions: [
              ...
              { label: 'Compare all tariffs', href: compareLabHref() }
            ],
            stats: [
    ...
    },
    {
      ...
      props: {
        ...
        action: { label: 'Compare all plans', href: compareLabHref() }
      }
    },
    {
      id: 'planCard',
    ...
    },
    {
      ...
      props: {
        ...
        compareHref: compareLabHref('prepaid'),
        actions: [
    ...
    },
    {
      ...
      props: {
        ...
        actions: [
          { label: 'Activate in Kabinetim', href: 'https://kabinetim.azercell.com/my/login', variant: 'primary' },
          { label: 'Compare with other plans', href: compareLabHref('prepaid') }
        ]
      }
    },
    {
    ...
    },
    {
      ...
      props: {
        links: [
          { label: 'Compare DigiMax with other plans', href: compareLabHref('prepaid'), variant: 'primary' },
